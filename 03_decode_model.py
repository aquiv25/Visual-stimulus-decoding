...
Task: predict stimulus change (Y=1) vs no-change (Y=0)
      from neural population activity (dF/F, 500 ms window).

Evaluation:
  - Stratified 5-fold cross-validation
  - Metrics: ROC-AUC, balanced accuracy, confusion matrix
  - Permutation test against H0: AUC = 0.5
"""
...
print(f"X: {X.shape}, y: {y.shape}, change rate: {y.mean():.2%}")
...
ax.set_title("Change Detection Decoder — ROC Curve (5-fold CV)")
