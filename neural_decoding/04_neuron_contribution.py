"""
Step 4: Identify which neurons contribute most to change detection decoding.

Methods:
  - RF built-in feature importance (mean decrease in impurity, MDI)
  - dF/F activity distributions: change vs no-change trials for top/bottom neurons
"""
...
# ---- Figure 3: dF/F distributions — change vs no-change ----
top5    = top20_idx[:5]
bottom5 = order[-5:]

fig, axes = plt.subplots(1, 2, figsize=(11, 4))
for ax, idx_group, label in zip(axes,
                                [top5, bottom5],
                                ["Top 5 neurons", "Bottom 5 neurons"]):
    change_vals   = X[y == 1][:, idx_group].mean(axis=1)
    nochange_vals = X[y == 0][:, idx_group].mean(axis=1)
    bins = np.linspace(
        min(change_vals.min(), nochange_vals.min()),
        max(change_vals.max(), nochange_vals.max()),
        20
    )
    ...
    ax.set_title(label)
    ax.legend()
fig.suptitle("Population Activity: Change vs No-Change Trials", fontsize=12)
plt.tight_layout()
